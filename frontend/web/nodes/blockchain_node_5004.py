...
class Blockchain():
    ...
    def init_chain_from_db(self,db):
        ...
        block_cursor.execute("SELECT block.id, block.timestamp, block.proof_of_work, block.previous_hash,hash.hash,miner.username, miner.created_at  FROM block INNER JOIN hash ON hash.block_id = block.id INNER JOIN user as miner ON  block.miner_id=miner.id ORDER BY block.id ASC")
