...
<?= GridView::widget([
    ...
    'columns' => [
        ...
        [
            ...
            'value' => function ($model) {
                if (extension_loaded('intl')) {
                    $subscriptionDate = date_create($model->subscribed);
                    ...
                    return Yii::t('user', ($model->subscribed) ? $subscriptionDate : 'Not set');
                } else {
                    return date('Y-m-d', $model->subscribed);
                }
            },
        ],
        [
        ...
        [
            ...
            'format' => 'raw',
            // 'visible' =>  \Yii::$app->user->can('Admin') || Yii::$app->user->can('manager') //Yii::$app->getModule('user')->enableConfirmation,
        ],
        [
            'header' => Yii::t('user', 'Subscription'),
            'value' => function ($model) {
                if ($model->isSubscribed) {
                    return '<div class="text-center">
                    <span class="text-warning">' . Yii::t('user', 'Subscribed') . '</span>
                    </div>';
                } else {
                    return Html::a(Yii::t('user', 'Resubscribe'), ['subscribe', 'id' => $model->id], [
                        'class' => 'btn btn-xs btn-warning btn-block',
                        'data-method' => 'post',
                        'data-confirm' => Yii::t('user', 'Are you sure you want to resubscribe this user?'),
                    ]);
                }
            },
            'format' => 'raw',
        ...
        [
            ...
            'visibleButtons' =>  [
                ...
                'update' =>\Yii::$app->user->can('Admin') || \Yii::$app->user->can('manager'),
            ],
            'buttons' => [
